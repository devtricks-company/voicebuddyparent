...
export function getColor(theme: DefaultTheme, color?: string) {
  ...
  const extractedColor =
    (get(theme.components, _color) as string) ?? (get(theme.palette, _color) as string) ?? _color

  return opacity ? convertHexToRGBA(extractedColor, Number(opacity)) : extractedColor
}
