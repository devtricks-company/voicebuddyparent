...
const { Consumer, Provider } = createContext<{ handleScroll: OnScroll } | undefined>(undefined)
...
export function CollapsingBackdrop(
  ...
  const { rightIcon, headerSize, title, caption, hideBack, children, _content, ...rest } = props
  const { headerStyle, opacityStyle, reverseOpacityStyle, handleScroll } = useCollapseAnimation()
  return (
    <BaseContainer flex={1} backgroundColor="container.overlay">
      <AnimatedHeader
        ...
        <HStack centerSpace>
          ...
          <AnimatedText style={opacityStyle} size="xr" font="normal" color="light">
            {title}
          </AnimatedText>
        </HStack>
        <AnimatedInformation style={reverseOpacityStyle} title={title} caption={caption} />
        {rightIcon}
      </AnimatedHeader>
